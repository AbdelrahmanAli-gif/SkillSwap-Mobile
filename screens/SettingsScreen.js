...
const SettingsScreen = () => {
  ...
  const settingsOptions = [
    ...
    },
    {
      ...
      icon: <MaterialIcons name="language" size={24} color={colors.colors.main} />,
      action: switchLang,
    },
    {
      id: "logout",
      label: t("SettingsScreen.logout"),
      icon: <MaterialIcons name="logout" size={24} color={colors.colors.main} />,
      action: handleLogout,
    },
  ]
      ...
      <View className="flex-1" style={{ direction: isRTL ? "rtl" : "ltr", marginTop: 30 }}>
        ...
        <View className="flex items-center justify-center py-2 px-4 rounded-xl mt-2">
          {currentUser.profilePicture ? (
            <Image source={{ uri: currentUser.profilePicture }} className="w-32 h-32 rounded-full" />
          ) : (
          ...
          <View className="flex-row items-center gap-2 mt-2">
            <Text className="text-2xl font-bold mt-2 text-main-color-light dark:text-main-color-dark">
              {currentUser.name}
            </Text>
